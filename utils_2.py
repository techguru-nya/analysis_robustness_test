...
def Checking_Signal_Value(file, datas):
    results = {}
...
# file = 'c:\\TSDE_Workarea\\ktt2yk\\Projects\\22MY_3A0A_CR-V\\Data\\1_VTC\\PET_KA_4WD\\Satoh\\20210421_0000.csv'
# datas = {'VARCodeForASW': [10, 11, 12, 15, 16, 17, 51, 52, 53, 54, 55, 56, 57, 58, 59, 70, 75, 76, 77],	
#         'VARCodeStatusForASW': [10, 11, 12, 15, 16, 17, 51, 52, 53, 54, 55, 56, 57, 58, 59, 70, 75, 76, 77],	
#         'VARStoredRawVariant': [10, 11, 12, 15, 16, 17, 51, 52, 53, 54, 55, 56, 57, 58, 59, 70, 75, 76, 77],
#         'VARStoredVariant': [10, 11, 12, 15, 16, 17, 51, 52, 53, 54, 55, 56, 57, 58, 59, 70, 75, 76, 77],
#         'VARVariantIndex': [10, 11, 12, 15, 16, 17, 51, 52, 53, 54, 55, 56, 57, 58, 59, 70, 75, 76, 77]}
# Checking_Signal_Value(file, datas)
...
def MakeInputForEst(CSV, CSV_out, resampling):
    ...
    if vRef.Check_Sigs(df, l_INPUT):
        ...
        l_Sig = ['v_FL', 'v_FR', 'v_RL', 'v_RR', 'Co_FA', 'vGiF', 'ayToF']
        for Sig in l_Sig:
            df_input[Sig] = df_input[Sig].rolling(window=50, center=True).mean()

        l_Sig = ['vGiF', 'ayToF']
        for Sig in l_Sig:
            df_input[Sig] = df_input[Sig].rolling(window=100, center=True).mean()
...
def Simulation_with_Matlab(CSV, Add, Dir, Mdl_Est, eng, override, resampling, zip_est_file=False):
    OUT = None
    l_Remove = []

    file_name, file_extension = os.path.splitext(os.path.basename(CSV))
    OUT_override = f'{Dir}{file_name}{Add}.csv'
    OUT_override_zip = f'{Dir}{file_name}{Add}.zip'
    ...
    if override == True or (os.path.exists(OUT_override) == False and os.path.exists(OUT_override_zip) == False):
        if os.path.exists(CSV):
            _ = MakeInputForEst(CSV, f'{Dir}simin.csv', resampling)
            
            # if os.path.exists(f'{Dir}simin.csv'):
            #     os.remove(f'{Dir}simin.csv')
            
            # shutil.copy(CSV, f'{Dir}simin.csv')
    
            if os.path.exists(f'{Dir}simin.csv'):
                OUT_Est = RunMatlab(Mdl_Est, f'{Dir}simout.csv', eng)
                l_Remove.append(f'{Dir}simin.csv')
                ...
                if os.path.exists(OUT_Est):
                    ...
                    print(f'No file: {OUT_Est}')
            else:
                no_file = f'{Dir}simin.csv'
...
def EstimateWheelTorque(d_File, Add, Dir, Mdl, matlab_version, override=True, resampling='200us'):
    ...
    for i, File in enumerate(d_File):
        ...
        #     try:
        #         Result = Simulation_with_Matlab(File, f'_{str(i)}_{Add}', Dir, Mdl, eng, override)
        ...
        try:
            Result = Simulation_with_Matlab(File, f'_{str(i)}_{Add}', Dir, Mdl, eng, override, resampling)
        except matlab.engine.MatlabExecutionError:
            Result = None
...
def VehicleCharacter(d_File, Add, Dir, Mdl, matlab_version, override=True):
    eng = matlab.engine.start_matlab(f"matlab.engine.shareEngine('{matlab_version}')")
    ...
    for i, File in enumerate(d_File):
        try:
            Result = Simulation_with_Matlab(File, f'_{Add}', Dir, Mdl, eng, override)
        except matlab.engine.MatlabExecutionError:
            Result = None
...
def Make_Input_for_Simulation(maps, maps_w_sampling={}, maps_w_average={}, constants={}, order=None, file=None, sampling_time='5ms'):
    data = {}
    
    for label in maps:
        data[label] = maps[label]

    for label in maps_w_average:
        input_data, _ = maps_w_average[label]
        data[label] = input_data
        
    df = pd.DataFrame(data)

    for label in constants:
        df[label] = constants[label]
    ...
    for label in maps_w_sampling:
        df_interpolated[label] = maps_w_sampling[label]

    for label in maps_w_average:
        _, n = maps_w_average[label]
        df_interpolated[label] = df_interpolated[label].rolling(window=n).mean()

    df_interpolated = df_interpolated[order]

    if file != None:
        df_interpolated.to_csv(file, index=False)
...
def Create_ABS_Target_Maps(initial_time, end_time, time_increase, time_decrease, time_delay, tar_depth, tar_base, sampling_time='5ms'):
    tar_FL = Create_ABS_Target_Map(initial_time, end_time, time_increase, time_decrease, 0, tar_depth, tar_base, sampling_time)
    tar_FR = Create_ABS_Target_Map(initial_time, end_time, time_increase, time_decrease, time_delay, tar_depth, tar_base, sampling_time)
    tar_RL = Create_ABS_Target_Map(initial_time, end_time, time_increase, time_decrease, 0, tar_depth, tar_base, sampling_time)
    tar_RR = Create_ABS_Target_Map(initial_time, end_time, time_increase, time_decrease, time_delay, tar_depth, tar_base, sampling_time)
    
    return tar_FL, tar_FR, tar_RL, tar_RR


def Create_ABS_Target_Map(initial_time, end_time, time_increase, time_decrease, time_delay, tar_depth, tar_base, sampling_time):
    times = [0, initial_time]
    tars = [tar_base, tar_base]
    
    time = initial_time + 0.0002
    tar = tar_base
    times.append(time)
    tars.append(tar)
    ...
    df_resampled = df.resample(sampling_time).mean()
    df_interpolated = df_resampled.interpolate(method='linear')
    # df_interpolated = df_resampled.interpolate(method='polynomial', order=2)

    # if file != None:
    #     df_interpolated.to_csv(file, index=False)

    # df_interpolated = df_interpolated[df_interpolated['TIME'] <= end_time]
    df_interpolated['mean'] = df_interpolated['X'].rolling(window=100).mean()

    # 数字部分を抽出
    time_str = sampling_time
    
    # 単位を確認して変換
    if time_str.endswith('ms'):
        number = float(time_str[:-2])
        time_in_seconds = number / 1000
    elif time_str.endswith('us'):
        number = float(time_str[:-2])
        time_in_seconds = number / 1000000
    else:
        time_in_seconds = number  # 他の単位がある場合の処理を追加できます
        
    n = int(end_time / time_in_seconds + 1)
    
    return list(df_interpolated['mean'])[:n]
...
# order = ['TIME', 'Co_FA', 'Throttle_in', 'Steer_in', 'pMC', 'p_FL', 'p_FR', 'p_RL', 'p_RR', 'V_tar', 'vGi_tar', 'Ay_tar', 'Auto_V', 'Auto_vGi', 'Auto_Ay', 'dev_tar_ABS', 'dev_tar_TCS', 'dev_tar_FL', 'dev_tar_FR', 'dev_tar_RL', 'dev_tar_RR']

# file = f'{DIR}simin.csv'
# initial_time = 30
# end_time = 40
# time_increase = 0.17
# time_decrease = 0.035
...
# tar_depth = 7.2
# tar_base = 3.6
# dev_tar_FL, dev_tar_FR, dev_tar_RL, dev_tar_RR = Create_ABS_Target_Maps(initial_time, end_time, time_increase, time_decrease, time_delay, tar_depth, tar_base, sampling_time='200us')
# maps = {'TIME': [0, 20, 25, 30, 30.2, 31, 40],
#         'Throttle_in': [100, 100, 100, 100, 0, 0, 0],
#         'pMC': [0, 0, 0, 0, 0, 200, 200],
#         'p_FL': [0, 0, 0, 0, 0, 200, 200],
#         'p_FR': [0, 0, 0, 0, 0, 200, 200],
#         'p_RL': [0, 0, 0, 0, 0, 200, 200],
#         'p_RR': [0, 0, 0, 0, 0, 200, 200],
#         'V_tar': [0, 27.78, 27.78, 27.78, 27.78, 27.78, 27.78],
#         'dev_tar_ABS': [10000, 10000, 10000, 10000, -10000, -10000, -10000]}
# maps_w_sampling = {'dev_tar_FL': dev_tar_FL,
#                    'dev_tar_FR': dev_tar_FR,
#                    'dev_tar_RL': dev_tar_RL,
#                    'dev_tar_RR': dev_tar_RR}
# constants = {'Co_FA': 0.5, 'Steer_in': 0, 
#              'vGi_tar': 0, 'Ay_tar': 0, 'Auto_V': 1, 'Auto_vGi': 0, 'Auto_Ay': 0,        
#              'dev_tar_TCS': 10}
# Inputs[file] = (maps, maps_w_sampling, constants, order)

# for file in Inputs:
#     maps, maps_w_sampling, constants, order = Inputs[file]
#     _ = Make_Input_for_Simulation(maps, maps_w_sampling=maps_w_sampling, constants=constants, order=order, file=file, sampling_time='200us')
...
# file = f'{DIR}simin.csv'
# maps = {'TIME': [0, 20, 25, 30, 30.0002, 30.2, 30.5, 35, 40],
#         'Throttle_in': [0, 0, 0, 0, 0, 100, 100, 100, 100],
#         'V_tar': [0, 0, 0, 0, 0, 100, 100, 100, 100],
#         'dev_tar_TCS': [100, 100, 100, 100, -10000, -10000, -10000, -10000, -10000]}
# maps_w_average = {'dev_tar_FL': ([0, 0, 0, 0, 0, 32, 10, 5.5, 5.5], 100),
#                   'dev_tar_FR': ([0, 0, 0, 0, 0, 32, 10, 5.5, 5.5], 100)}
# constants = {'Co_FA': 1, 'Steer_in': 0, 
#              'pMC': 0, 'p_FL': 0, 'p_FR': 0, 'p_RL': 0, 'p_RR': 0, 
#              'vGi_tar': 0, 'Ay_tar': 0, 'Auto_V': 1, 'Auto_vGi': 0, 'Auto_Ay': 0,        
#              'dev_tar_ABS': 10000, 'dev_tar_RL': 0, 'dev_tar_RR': 0}
# Inputs[file] = (maps, maps_w_average, constants, order)

# for file in Inputs:
#     maps, maps_w_average, constants, order = Inputs[file]
#     _ = Make_Input_for_Simulation(maps, maps_w_average=maps_w_average, constants=constants, order=order, file=file, sampling_time='200us')
...
def Resampling(Files, override=False, list=False, sampling_time='5ms'):
    if list == False:
        Out = Files
    else:
        Out = []
    ...
    for File in Files:
        ...
        if os.path.exists(File_new) == False or override == True:
            ...
            if list == False:
                ...
                Out.append(f'{root}_{sampling_time}.csv')
